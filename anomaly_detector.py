...
class AnomalyDetector:
    ...
    def train(self, use_pca=False):
        ...
        if len(self.feature_history) < 100:
            self.logger.warning(f"훈련 데이터가 부족합니다. 현재: {len(self.feature_history)}, 최소 필요: 100")
            return False
        ...
        try:
            ...
            if len(features_array) < 100:
                self.logger.warning("유효한 특징 데이터가 부족합니다.")
                return False
            ...
            if self.use_pca:
                ...
                self.logger.info(f"PCA 적용: {features_array.shape[1]} -> {normalized_features.shape[1]} 차원")
            
            # 이상 검출 모델 훈련
            self.detector.fit(normalized_features)
            self.is_trained = True
            
            self.logger.info(f"모델 훈련 완료: {len(features_array)}개 샘플, 알고리즘: {self.algorithm}")
            return True
            
        except Exception as e:
            self.logger.error(f"훈련 중 오류 발생: {e}")
            return False
    ...
    def detect_anomaly(self, features_window):
        ...
        try:
            ...
            is_anomaly = anomaly_ratio > 0.3  # 30% 이상이 이상으로 판정되면 이상
...
class AdaptiveAnomalyDetector(AnomalyDetector):
    ...
    def detect_anomaly_adaptive(self, features_window, true_label=None):
        ...
        if len(self.recent_scores) >= 50:
            ...
            # 임계값 범위 제한
            self.adaptive_threshold = max(0.1, min(0.9, self.adaptive_threshold))
    ...
    def adapt_model(self):
        ...
        if false_positive_rate > 0.2:
            self.adaptive_threshold = min(0.9, self.adaptive_threshold * 1.1)
            self.logger.info(f"오탐지율 높음 -> 임계값 상향: {self.adaptive_threshold:.3f}")
        
        # 미탐지율이 높으면 임계값 하향 조정
        elif false_negative_rate > 0.2:
            self.adaptive_threshold = max(0.1, self.adaptive_threshold * 0.9)
            self.logger.info(f"미탐지율 높음 -> 임계값 하향: {self.adaptive_threshold:.3f}")
        
        # 성능이 현저히 떨어지면 재훈련 고려
        metrics = self.get_performance_metrics()
        if metrics['f1'] < 0.6 and metrics['total_samples'] > 100:
